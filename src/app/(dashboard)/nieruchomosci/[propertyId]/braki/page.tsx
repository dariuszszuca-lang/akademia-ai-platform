...
import { buildOpenIssues } from '@/features/properties/open-issues'
import {
...
export default async function PropertyIssuesPage({
  ...
  const [project, facts, sources, proposals] = workspace
  const issues = buildOpenIssues({ facts, sources, proposals })
        ...
        <div className="relative">
          ...
          <div className="mt-5 flex flex-col gap-6 lg:flex-row lg:items-end lg:justify-between">
            ...
            <dl className="grid grid-cols-3 divide-x divide-white/10 overflow-hidden rounded-2xl border border-white/10 bg-white/[0.04]">
              <Metric label="Otwarte" value={issues.length} />
              <Metric
                label="Konflikty"
                value={conflictCount}
                alert={conflictCount > 0}
              />
              <Metric label="Braki" value={missingCount} />
            </dl>
          </div>
        </div>
      </header>

      <PropertyWorkspaceTabs propertyId={project.id} active="issues" />

      <OpenIssuesBoard propertyId={project.id} issues={issues} />
    </div>
  )
}
