...
export default function CoursePage() {
  ...
  return (
    <div className="flex gap-0 -mx-4 sm:-mx-6 -mt-6">
      {/* Left sidebar */}
      <div className="w-80 flex-shrink-0 border-r border-border bg-card overflow-y-auto" style={{ maxHeight: "calc(100vh - 120px)" }}>
        {/* Back + title */}
        <div className="p-5 border-b border-border">
          <Link href="/classroom" className="flex items-center gap-1 text-xs text-foreground/40 hover:text-foreground transition-colors mb-3">
          ...
          {course.modules.map((mod) => {
            ...
            return (
              <div key={mod.id}>
                ...
                >
                  <span className="text-sm font-semibold text-foreground truncate pr-2">{mod.title}</span>
                  <svg
        ...
        {selectedLesson ? (
          <div className="p-8 max-w-3xl">
            ...
            {selectedLesson.content ? (
              <div className="text-foreground/80 leading-relaxed whitespace-pre-line">{selectedLesson.content}</div>
            ) : (
